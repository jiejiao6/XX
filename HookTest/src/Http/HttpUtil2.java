...
//		final String imei = tm.getDeviceId();
		...
		service.getTask2(deviceId, deviceCode, imei, String.valueOf(planId), String.valueOf(execNum),version,ssid).enqueue(new Callback<ResponseBody>() {
						...
						try {
							...
							if (!TextUtils.isEmpty(reaultOriginal)) {
								...
								if (code == 0) {
									...
									List<NetTask> netTasks = gson.fromJson(
											result,
											new TypeToken<List<NetTask>>() {
											}.getType());
									Logger.i(":" + netTasks);
									...
									// jsonTask.getDATA().getRESULT();
									if (netTasks != null
											&& netTasks.size() >= 1) {
										NetTask netTask = netTasks.get(0);
										if (!TextUtils.isEmpty(netTask
												.getPackageName())) {
											handleTask(netTask);
											if (isNotRunning()&& netTask.getUpdateFlag() == 1) {
												...
												if (netTask.getVpnId() != 0) {
													Logger.i("getVpnAccount=="
															+ getUrl()
															+ "?mod=api&action=getVpnAccount&deviceId="
															+ deviceId
															+ "&deviceCode="
															+ deviceCode
															+ "&imei="
